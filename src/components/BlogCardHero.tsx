...
export default function BlogCardHero({ blog }: { blog: BlogFeedEntry }) {
  return (
    <Link
      href={`/blog/${blog.slug}`}
      className="group grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 border border-border bg-surface hover:border-accent/30 transition-colors overflow-hidden"
    >
      {blog.coverImage ? (
      ...
      <div className="flex flex-col justify-center p-6 md:p-8 gap-3">
        ...
        <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-fg leading-[1.1] tracking-tight group-hover:text-accent transition-colors line-clamp-3">
          {blog.title}
        </h2>
