/**
 * Content-safety checker for the Node.js "ultra" answer files.
 *
 ...
 * technology='nodejs' row (the augment script matches on exact title, so a
 * typo silently becomes a no-op).
 *
 *   npx tsx scripts/check-node-ultra.ts [--no-db] [--file <name>]
 */
...
function checkStructure(title: string, file: string, a: Augment) {
  ...
  for (const ex of a.examples ?? []) {
    ...
    // Node examples render as static, syntax-highlighted code (no playground) —
    // runnable must be false or omitted, and every example was executed with
    // `node` before being pasted in (§4), not asserted as "standard syntax".
    if (ex.runnable === true)
      fail(title, file, "example-runnable-true", `"${ex.label ?? "?"}" sets runnable:true — Node has no code-runner UI, always false/omitted`);
  }
}
...
async function main() {
  ...
  const files = readdirSync(dir)
    .filter((f) => /^node-augments-ultra.*\.(ts|json)$/.test(f))
    .filter((f) => !onlyFile || f === onlyFile)
    .sort();

  if (!files.length) { console.log("No node-augments-ultra-*.ts files yet."); return; }
  ...
  if (!NO_DB) {
    ...
    try {
      const rows = await prisma.prepQuestion.findMany({ where: { technology: "nodejs" }, select: { title: true } });
      const known = new Set(rows.map((r) => r.title));
      for (const [t, f] of seen) if (!known.has(t)) fail(t, f, "title-not-in-db", "no technology='nodejs' row with this exact title — augment would be a silent no-op");
    } catch (e) {
