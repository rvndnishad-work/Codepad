/**
 * Confirms what actually landed in the database for one JS coding batch:
 * every title matched a row, and each row carries the answer, the Question
 * Body, a short seoDescription, at least one genuinely runnable example,
 * and the required thought-process sections (clarifying questions, thought
 * process, complexity, edge cases).
 *
 * Usage: npx tsx scripts/db-check-js-coding.mts js-coding-augments-1.ts
 */
...
if (!file) {
  console.error("Usage: npx tsx scripts/db-check-js-coding.mts <js-coding-augments-NN.ts>");
  process.exit(1);
}
...
for (const r of rows) {
  ...
  if (!/clarifying question/i.test(a)) problems.push("no clarifying questions section");
  if (!/thought process/i.test(a)) problems.push("no Thought Process section");
  if (!/complexity/i.test(a)) problems.push("no complexity analysis");
  if (!/edge case/i.test(a)) problems.push("no edge cases section");
  ...
  const examples = r.examplesData ? (JSON.parse(r.examplesData) as { runnable?: boolean }[]) : [];
  if (!examples.length) problems.push("no examples");
  const runnable = examples.filter((e) => e.runnable !== false).length;
  if (examples.length && runnable === 0) problems.push("no runnable example (all runnable:false)");
  if (problems.length) bad++;
  console.log(
    `${problems.length ? "✖" : "✔"} ${(a.length / 1024).toFixed(1)}KB  ` +
      `desc=${r.description?.length ?? 0} seo=${r.seoDescription?.length ?? 0} ex=${examples.length} run=${runnable}  ${r.slug}`,
  );
