  ...
  return (
    <div ref={ref} className="relative inline-block">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        disabled={pending}
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-xs font-semibold transition-colors disabled:opacity-60 ${STAGE_TONES[stage]}`}
      >
          ...
          <div className="p-1">
            {PIPELINE_STAGES.map((s) => (
              ...
              >
                ...
                {s === stage && <Check className="w-3.5 h-3.5 text-secondary" />}
              </button>
            ))}
          </div>
        </div>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-md border border-border bg-bg text-fg text-xs focus:outline-none focus:border-secondary/40 resize-none"
                />
              </div>
              <button
                type="button"
                ...
                className="w-full py-2 rounded-md bg-danger/10 border border-danger/30 hover:bg-danger/15 text-danger text-xs font-semibold transition-colors disabled:opacity-50"
              >
                Confirm rejection
              </button>
            </div>
