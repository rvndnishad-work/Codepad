 ...
 *
 * Usage: npx tsx scripts/db-check-nextjs-ultra.mts nextjs-augments-ultra-10.ts
 */
...
if (!file) {
  console.error("Usage: npx tsx scripts/db-check-nextjs-ultra.mts <nextjs-augments-ultra-NN.ts>");
  process.exit(1);
}
...
const rows = await prisma.prepQuestion.findMany({
  where: { technology: "nextjs", title: { in: titles } },
