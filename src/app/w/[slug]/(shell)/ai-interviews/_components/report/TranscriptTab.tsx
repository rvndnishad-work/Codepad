...
import { transcriptStamps } from "@/lib/ai-interview/report-extras";
 ...
 * older ones read as one continuous thread. Each message shows its time from
 * the start of the screening, or its place in the thread when no time was
 * recorded. A time links to its message, so it can be pasted into a note.
 */
export default function TranscriptTab({
  r,
}: {
  r: Pick<ReportData, "chat" | "status" | "startedAt"> & { candidate: { name: string }; rounds: { id: string; title: string }[] };
}) {
  const [q, setQ] = useState("");
  const stamps = useMemo(() => transcriptStamps(r.chat, r.startedAt), [r.chat, r.startedAt]);
  const shown = useMemo(() => {
  ...
  const timed = stamps.some((s) => s.timed);
        ...
        <div className="flex flex-col gap-0.5">
          ...
          {r.chat.length > 0 && (
            <p className="text-xs text-subtle">
              {timed ? "Times are from the start of the screening." : "This screening did not record times, so messages are numbered in order."}
            </p>
          )}
        </div>
        <label className="relative w-full sm:w-60 print:hidden">
      ...
      <div className="rounded-xl border border-border bg-surface p-4 sm:p-6 flex flex-col gap-5">
        {r.chat.length === 0 ? (
          ...
          shown.map((m, idx) => {
            ...
            const stamp = stamps[m.i];
            return (
              <div key={m.i} id={`msg-${m.i + 1}`} className="flex flex-col gap-5 scroll-mt-24">
                {divider && (
                ...
                <div className={`flex gap-3 items-start ${ai ? "" : "flex-row-reverse"}`}>
                  ...
                  <div className={`max-w-[85%] sm:max-w-[560px] flex flex-col gap-1 ${ai ? "items-start" : "items-end"}`}>
                    <span className={`flex items-center gap-2 text-xs text-subtle ${ai ? "" : "flex-row-reverse"}`}>
                      <span>{ai ? "AI interviewer" : firstName}</span>
                      {stamp && (
                        <a
                          href={`#msg-${m.i + 1}`}
                          className="font-mono tabular-nums text-secondary-soft hover:underline"
                          title={stamp.timed ? "Time from the start of the screening" : "Place in the conversation"}
                        >
                          {stamp.label}
                        </a>
                      )}
