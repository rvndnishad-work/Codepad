/**
 * Content-safety checker for the Next.js "ultra" answer files.
 *
 ...
 * technology="nextjs" row (the augment script matches on exact title, so a
 * typo silently becomes a no-op).
 *
 *   npx tsx scripts/check-nextjs-ultra.ts [--no-db] [--file <name>]
 */
...
const internalLinks: { title: string; file: string; slug: string }[] = [];
...
function checkCard(title: string, file: string, md: string) {
  ...
  if (!card) {
    fail(title, file, "card-missing", "no 'How to Answer in an Interview' card (§7 is mandatory for Next.js)");
    return;
  }
...
function checkStructure(title: string, file: string, a: Augment) {
  ...
  for (const ex of a.examples ?? []) {
    // Next.js code (server components, route handlers, next/* imports) cannot run in the
    // Sandpack playground, so examples are static, highlighted snippets.
    if (ex.runnable !== false)
      fail(title, file, "example-runnable", `"${ex.label ?? "?"}" must set runnable: false (Next.js code cannot run in the playground)`);
    if (!ex.label) soft(title, file, "example-no-label", "example without a label");
  }

  // Cross-links: the PASTE_..._URL_HERE placeholder (CLAUDE.md §11) is resolved by nothing in
  // this app, so a leftover one is a dead link. Internal links must point at a real question.
  if (/PASTE_[A-Z0-9_]+_URL_HERE/.test(md))
    fail(title, file, "dead-placeholder-link", "PASTE_..._URL_HERE placeholder left in the answer — link to /interview-question/<slug> instead");
  for (const m of md.matchAll(/\]\(\/interview-question\/([a-z0-9-]+)\)|href="\/interview-question\/([a-z0-9-]+)"/g))
    internalLinks.push({ title, file, slug: (m[1] ?? m[2]) as string });
}
...
async function main() {
  ...
  const files = readdirSync(dir)
    .filter((f) => /^nextjs-augments-ultra.*\.(ts|json)$/.test(f))
    .filter((f) => !onlyFile || f === onlyFile)
    .sort();

  if (!files.length) { console.log("No nextjs-augments-ultra-*.ts files yet."); return; }
  ...
  if (!NO_DB) {
    ...
    try {
      const rows = await prisma.prepQuestion.findMany({ where: { technology: "nextjs" }, select: { title: true } });
      const known = new Set(rows.map((r) => r.title));
      for (const [t, f] of seen) if (!known.has(t)) fail(t, f, "title-not-in-db", "no technology='nextjs' row with this exact title — augment would be a silent no-op");
      // Internal links must resolve to a live question page (any technology).
      const slugs = new Set((await prisma.prepQuestion.findMany({ select: { slug: true } })).map((r) => r.slug));
      for (const l of internalLinks) if (!slugs.has(l.slug)) fail(l.title, l.file, "dead-internal-link", `/interview-question/${l.slug} does not exist`);
    } catch (e) {
