/**
 * Renders each JS ultra answer through the SAME plugin chain the question
 ...
 *
 * Usage:
 *   npx tsx scripts/render-check-js-ultra.mts                 (all files)
 *   npx tsx scripts/render-check-js-ultra.mts --file js-augments-ultra-1.ts
 */
...
const files = readdirSync(DATA_DIR)
  .filter((f) => /^js-augments-ultra-\d+\.ts$/.test(f))
  ...
  for (const a of augments as { title: string; answer?: string; description?: string; examples?: { runnable?: boolean }[] }[]) {
    ...
    console.log(
      `  ${problems.length ? "✖" : "✔"} svg=${svgs.length} card=${card ? "y" : "N"} ` +
        `tables=${tables} pin=${pins} run=${runnable}  ${a.title.slice(0, 50)}`,
    );
