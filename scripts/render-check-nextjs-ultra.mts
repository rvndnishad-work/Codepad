 ...
 * static checker in scripts/check-nextjs-ultra.ts reads the SOURCE; this one
 * proves the source survives the renderer — that raw SVG and the amber card
 * come out as elements rather than escaped text.
 *
 * Usage:
 *   npx tsx scripts/render-check-nextjs-ultra.mts                 (all files)
 *   npx tsx scripts/render-check-nextjs-ultra.mts --file nextjs-augments-ultra-10.ts
 */
...
const files = readdirSync(DATA_DIR)
  .filter((f) => /^nextjs-augments-ultra-\d+\.ts$/.test(f))
