/**
 * Content-safety checker for practical JS coding-interview answer files
 * (technology='javascript-coding').
 *
 * Shares every low-level rule with scripts/check-js-ultra.ts (SVG safety,
 * §7 interview-card inline-color rule, div balance, seoDescription length,
 * at-least-one-runnable-example) — those were each discovered by actually
 * breaking the renderer and apply identically here, since the DB fields and
 * rendering pipeline are the same. What's DIFFERENT is the required
 * structure: this bank is thought-process-first coding problems, not
 * definitional Q&A, so it checks for a distinct section set (clarifying
 * questions, thought process, complexity analysis, edge cases table)
 * instead of 📌 Interview term callouts.
 *
 *   npx tsx scripts/check-js-coding.ts [--no-db] [--file <name>]
 */
...
// ── SVG checks (identical to check-js-ultra.ts) ────────────────────────────
...
// ── CSS shorthand-typo check — crashed jsdom's own parser in a prior project ─

function checkCssTypo(title: string, file: string, md: string) {
  if (/solid#/.test(md))
    fail(title, file, "css-typo", "found 'solid#' — a missing space before a hex color breaks jsdom's own CSS parser at render-check time");
}

// ── Interview card checks (§7, identical styling rule to check-js-ultra.ts) ─
...
function checkCard(title: string, file: string, md: string) {
  ...
  if (!card) {
    fail(title, file, "card-missing", "no 'How to Answer in an Interview' process card");
    return;
  }
  ...
  if (qa < 4) soft(title, file, "card-followups", `${qa} follow-up Q&A pairs (want 4-5 realistic interviewer follow-ups)`);
  const script = (card.match(/<blockquote/g) || []).length;
  if (script < 5) soft(title, file, "card-script", `${script} numbered process steps (want 5)`);
}

// ── Structure + Question Body — the thought-process-specific requirements ──

function checkStructure(title: string, file: string, a: Augment) {
  const md = a.answer ?? "";
  if (!md) { fail(title, file, "no-answer", "answer is empty"); return; }

  if (!/clarifying question/i.test(md))
    fail(title, file, "no-clarifying-questions", "no clarifying-questions section — central to this bank's thought-process focus");
  if (!/thought process/i.test(md))
    fail(title, file, "no-thought-process", "no 'Thought Process' section (brute-force -> optimization reasoning)");
  if (!/complexity/i.test(md))
    fail(title, file, "no-complexity", "no time/space complexity analysis");
  if (!/edge case/i.test(md))
    fail(title, file, "no-edge-cases", "no edge-cases section/table");
  ...
  if (!/common pitfall/i.test(md)) soft(title, file, "no-pitfalls", "no Common Pitfalls section");
  ...
  if (!a.description) fail(title, file, "no-description", "no Question Body");
  else {
    ...
    if (!/example/i.test(a.description))
      soft(title, file, "description-no-examples", "Question Body has no worked input/output example");
  }
  ...
  if (!a.examples?.length) { fail(title, file, "no-example", "no code example"); return; }
  const runnableCount = (a.examples ?? []).filter((ex) => ex.runnable !== false).length;
  if (runnableCount === 0)
    fail(title, file, "no-runnable-example", "every example sets runnable:false — javascript-coding questions must have at least one genuinely runnable example");
...
async function main() {
  ...
  if (!files.length) { console.log("No js-coding-augments-*.ts files yet."); return; }
  ...
  for (const f of files) {
    ...
    for (const a of arr) {
      ...
      checkCssTypo(a.title, f, a.answer ?? "");
  ...
  if (!NO_DB) {
    ...
    try {
      const rows = await prisma.prepQuestion.findMany({ where: { technology: "javascript-coding" }, select: { title: true } });
      const known = new Set(rows.map((r) => r.title));
      for (const [t, f] of seen) if (!known.has(t)) fail(t, f, "title-not-in-db", "no technology='javascript-coding' row with this exact title — augment would be a silent no-op");
    } catch (e) {
