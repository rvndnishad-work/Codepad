 ...
 * static checker in scripts/check-node-ultra.ts reads the SOURCE; this one
 * proves the source survives the renderer — that raw SVG and the amber card
 * come out as elements rather than escaped text.
 *
 * Usage:
 *   npx tsx scripts/render-check-node-ultra.mts                 (all files)
 *   npx tsx scripts/render-check-node-ultra.mts --file node-augments-ultra-1.ts
 */
...
const files = readdirSync(DATA_DIR)
  .filter((f) => /^node-augments-ultra-\d+\.ts$/.test(f))
