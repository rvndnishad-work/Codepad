  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-20">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-8 rounded-3xl border border-border bg-[#0A0A0A] p-1 overflow-hidden group shadow-2xl">
        ...
        {/* Feature Matrix */}
        {FEATURES.map((f, i) => (
          <div key={i} className="md:col-span-4 rounded-3xl border border-border bg-panel p-6 hover:bg-elevated transition-colors group">
             <div className="w-12 h-12 rounded-2xl border border-white/5 bg-white/[0.02] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform" style={{ borderColor: `${f.color}33` }}>
        ...
        {/* Quick Start Grid */}
        {QUICK_STARTS.map((q) => (
           <Link key={q.id} href={`/play?template=${q.id}`} className="md:col-span-3 rounded-2xl border border-border bg-panel/40 p-4 hover:border-[#FFE600]/40 hover:bg-panel transition-all group">
              <div className="flex items-center justify-between mb-4">
                <div className="w-10 h-10 rounded-xl bg-surface border border-border grid place-items-center group-hover:scale-110 transition-transform">
