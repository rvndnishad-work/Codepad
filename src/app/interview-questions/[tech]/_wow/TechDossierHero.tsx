 ...
 * giant watermark glyph, mono stat readouts and a difficulty spectrum strip.
...
export default function TechDossierHero({
  ...
  return (
    <section ref={root} data-dark-hero className="wow-noise relative -mt-16 overflow-hidden bg-[#08080f] text-white">
      ...
      {/* watermark glyph */}
      <div aria-hidden className="td-mark pointer-events-none absolute -right-8 top-1/2 hidden -translate-y-1/2 opacity-[0.14] md:block">
        <TechSvg tech={tech} className="h-72 w-72" />
      </div>

      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-10 pt-24 md:pt-28">
        {/* breadcrumb back to the archive */}
        <nav aria-label="Breadcrumb" className="td-fade flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-[0.2em] text-white/45">
        ...
        <p className="td-fade mt-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.06] px-4 py-1.5 font-mono text-[11px] uppercase tracking-[0.2em] text-white/75 backdrop-blur-md">
          <FileQuestion className="h-3.5 w-3.5" style={{ color: hex }} />
          Sector file // {tech}
        </p>

        <h1 className="wow-font-display mt-4 text-5xl leading-[0.95] drop-shadow-[0_2px_16px_rgba(0,0,0,0.95)] sm:text-6xl md:text-7xl">
        ...
        <p className="td-fade mt-4 max-w-2xl text-[15px] font-medium leading-relaxed text-white/70">
          {tagline}
        </p>

        {/* stat readouts + spectrum */}
        <div className="td-fade mt-7 flex flex-wrap items-center gap-x-8 gap-y-3 font-mono text-[11px] uppercase tracking-[0.18em] text-white/55">
