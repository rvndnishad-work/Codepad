/**
 * Confirms what actually landed in the database for one JS ultra batch: every
 * title matched a row, and each row carries the answer, the Question Body,
 * a short seoDescription, and at least one genuinely runnable example.
 *
 * Usage: npx tsx scripts/db-check-js-ultra.mts js-augments-ultra-1.ts
 */
...
if (!file) {
  console.error("Usage: npx tsx scripts/db-check-js-ultra.mts <js-augments-ultra-NN.ts>");
  process.exit(1);
}
...
for (const r of rows) {
  ...
  console.log(
    `${problems.length ? "✖" : "✔"} ${(a.length / 1024).toFixed(1)}KB  ` +
      ...
      `seo=${r.seoDescription?.length ?? 0} ex=${examples.length} run=${runnable}  ${r.slug}`,
  );
